...
function sleep(a){return new Promise(function(b){return setTimeout(b,a)})}
$(function(){function a(){var e,g,d,l;return $jscomp.asyncExecutePromiseGeneratorProgram(function(c){1==c.nextAddress&&(e=$(window).width(),d=g=0,l=b.find("img"),l.each(function(){var f=$(this).one("load",function(){var h=this.naturalWidth,k=this.naturalHeight;k=Math.min(k,k*e/h);f.width(Math.min(h,e));f.height(k);d=Math.max(k,d);g++})}));if(3!=c.nextAddress)return g===l.length?c.jumpTo(3):c.yield(sleep(100),2);b.find(".carousel-item").css("height",d+"px");c.jumpToEnd()})}var b=$("#carousel");b.on("mouseenter",
function(){$(this).carousel("pause")}).on("mouseleave",function(){$(this).carousel("cycle")}).on("slide.bs.carousel",function(e){});a();$(window).on("resize orientationchange",a)});
